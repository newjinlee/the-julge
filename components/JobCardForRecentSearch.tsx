  ...
  return (
    <div
      ...
      <div className="w-full h-40 rounded-xl overflow-hidden justify-center items-center flex">
        <Image
          className={`w-full h-full object-cover ${job.closed ? 'brightness-50' : ''}`}
          src={job.shop.item.imageUrl}
          alt={job.shop.item.name}
          width={597}
          height={543}
        />
        {job.closed && (
      ...
      <div className="flex flex-col justify-between items-start gap-4 flex-grow">
        <div className="flex flex-col justify-start items-start gap-2">
          <div
            className={`text-xl font-bold font-['Spoqa Han Sans Neo'] ${isClosed ? 'text-stone-300' : 'text-gray-900'}`}>
            {job.shop.item.name}
          </div>
          <div className="justify-start items-center gap-1.5 inline-flex">
            ...
            <div
              className={`text-sm font-normal font-['Spoqa Han Sans Neo'] leading-snug ${isClosed ? 'text-stone-300' : 'text-zinc-500'}`}>
        ...
        <div className="self-stretch gap-3 flex justify-between items-center">
          <div
            className={`text-2xl font-bold font-['Spoqa Han Sans Neo'] tracking-wide ${isClosed ? 'text-stone-300' : 'text-gray-900'}`}>
            {job.hourlyPay.toLocaleString('ko-KR')}원
          </div>
          <div
            className={`w-[168px] p-3 ${isClosed ? 'bg-stone-300' : 'bg-red-500'} rounded-[20px] justify-start items-center gap-1.5 flex`}>
